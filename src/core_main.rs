                ...
                if crate::platform::is_installed() && is_root() {
                    ...
                    if let Ok(lic) = crate::custom_server::get_custom_server_from_string(&name) {
                        if !lic.host.is_empty() {
                            // iRidi build:
                            // Server parameters are compiled in.
                            // Do not persist key/custom-rendezvous-server/api-server/relay-server to RustDesk.toml.
                            let _ = lic;
                            println!("Custom server config is disabled in this build.");
                        }
                    }
                } else {
                    println!("Installation and administrative privileges required!");
