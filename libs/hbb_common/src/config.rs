...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("iRidiDesk".to_owned());
...
pub const LINK_DOCS_HOME: &str = "https://www.iridi.com/";
pub const LINK_DOCS_X11_REQUIRED: &str = "https://www.iridi.com/";
pub const LINK_HEADLESS_LINUX_SUPPORT: &str =
    "https://www.iridi.com/";

lazy_static::lazy_static! {
    pub static ref HELPER_URL: HashMap<&'static str, &'static str> = HashMap::from([
        ("irididesk docs home", LINK_DOCS_HOME),
        ("irididesk docs x11-required", LINK_DOCS_X11_REQUIRED),
        ("irididesk x11 headless", LINK_HEADLESS_LINUX_SUPPORT),
        ]);
}
...
impl Config {
    ...
    pub fn update_latency(host: &str, latency: i64) {
        ...
        if !host.is_empty() {
            ...
            if host != config.rendezvous_server {
                log::debug!("Update rendezvous_server in memory");
    ...
    pub fn set_options(mut v: HashMap<String, String>) {
		if IRIDI_VENDOR_LOCKED {
            v.remove("custom-rendezvous-server");
            v.remove("relay-server");
            v.remove("api-server");
            v.remove("key");
    ...
    pub fn get_option(k: &str) -> String {
		if IRIDI_VENDOR_LOCKED {
    		match k {
        		"custom-rendezvous-server" => return "".to_string(),
